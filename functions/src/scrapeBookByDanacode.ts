  ...
  for (const catnum of candidates) {
    ...
    $("span, div, p, td, li, label, strong, b, dt, dd").each((_i, el) => {
      ...
      if (text.length > 2 && text.length < 200) {
        // "Label: Value" pattern
        const colonMatch = text.match(
          /^(עמודים|משקל|שנת הוצאה|תרגום|מסת"ב|מסת״ב|ISBN)\s*:\s*(.+)$/
        );
        if (colonMatch) {
          labelMap[colonMatch[1]] = colonMatch[2].trim();
        }
  ...
  try {
    ...
    if (opusData) {
      if (!result) {
        ...
        if (!result.numberOfPages && opusData.numberOfPages) result.numberOfPages = opusData.numberOfPages;
        if (!result.weight && opusData.weight) result.weight = opusData.weight;
        if (!result.translationPublishingYear && opusData.translationPublishingYear) result.translationPublishingYear = opusData.translationPublishingYear;
        if (!result.translatedBy && opusData.translatedBy) result.translatedBy = opusData.translatedBy;
        if (!result.coverImageUrl && opusData.coverImageUrl) result.coverImageUrl = opusData.coverImageUrl;
